...
def read_conll_file(data, path):
    ...
    return data
def merge(dataset1,data):
    
    data.update(dataset1)
    #merge_dataset = dict(dataset1.items()+dataset2.items())
    return data
...
class CoNLLDataset:
    ...
    def __init__(self, path, person_path, conll_path):
        ...
        with open(path+'/test3_finalinputtestoldwiki2.json','r') as f:
            self.bt = json.load(f)
        # with open(path+'/train3_finalinputtestoldwiki2.json','r') as f:
        #     dataset1 = json.load(f)
        # self.bt = merge(self.bt,dataset1)
        print('process coref')
        ...
        with open(path+'/train3_finalinputtestoldwiki2.json','r') as f:
            dataset1 = json.load(f)
        new_dataset = {}
        for i,(k,v) in enumerate(dataset1.items()):
            new_dataset[k]=v
            if i==50: #edit here
                break
        merge(new_dataset, read_conll_file(self.train, conll_path + '/AIDA/aida_train.txt'))
...
if __name__ == "__main__":
    path = '/Users/maggiemin/Documents/gqp_bt/data/generated/test_train_data'
    conll_path = '/Users/maggiemin/Documents/gqp_bt/data/basic_data/test_datasets'
    person_path = '/Users/maggiemin/Documents/gqp_bt/data/basic_data/p_e_m_data/persons.txt'
    import json
    dataset = CoNLLDataset(path, person_path, conll_path)
    print(dataset.testA.keys())

    #with open('/Users/maggiemin/Documents/testA', "w") as file2:
        #json.dump(dataset.testA, file2)
    for docname, content in dataset.testA.items():
        for m in content:
            print (m.keys())
            print (m['mention'])
            print (m['context'])
